...
export default function Lessons() {
  ...
  useEffect(() => {
    ...
    if (!isGuest) {
      api.get("/auth/bookmarks").then(res => {
  ...
  const toggleBookmark = async (e, lessonId) => {
    e.stopPropagation();
    if (isGuest) { nav("/register"); return; }
    try {
      const res = await api.post(`/auth/bookmark/${lessonId}`);
      setBookmarks(prev => res.data.bookmarked
        ? [...prev, lessonId]
  ...
  const safeContent = (html) => DOMPurify.sanitize(
      ...
      <div style={{ maxWidth: 800, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
          <button onClick={() => setSelected(null)}
            style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.6rem 1.25rem', background: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.8)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '10px', cursor: 'pointer', fontWeight: 600 }}>
            ← Back to Lessons
          </button>
          {!isGuest && (
            <button onClick={(e) => toggleBookmark(e, selected._id)}
              style={{ padding: '0.6rem 1.25rem', background: bookmarks.includes(selected._id) ? 'rgba(245,158,11,0.2)' : 'rgba(255,255,255,0.08)', color: bookmarks.includes(selected._id) ? '#f59e0b' : 'rgba(255,255,255,0.7)', border: `1px solid ${bookmarks.includes(selected._id) ? '#f59e0b' : 'rgba(255,255,255,0.1)'}`, borderRadius: '10px', cursor: 'pointer', fontWeight: 600 }}>
              {bookmarks.includes(selected._id) ? '🔖 Bookmarked' : '🔖 Bookmark'}
            </button>
          )}
        </div>
        ...
        <div style={{ background: 'rgba(31,41,55,0.9)', borderRadius: '20px', overflow: 'hidden', border: '1px solid rgba(59,130,246,0.15)', marginBottom: '1.5rem' }}>
          ...
          <div style={{ padding: '2rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.6rem', flexWrap: 'wrap' }}>
              <span style={{ background: 'rgba(59,130,246,0.15)', color: '#93c5fd', padding: '0.3rem 0.8rem', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 600, border: '1px solid rgba(59,130,246,0.2)' }}>📊 {selected.level}</span>
              <span style={{ background: 'rgba(139,92,246,0.15)', color: '#c4b5fd', padding: '0.3rem 0.8rem', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 600, border: '1px solid rgba(139,92,246,0.2)' }}>⏱️ {selected.duration}</span>
              {selected.isFree && <span style={{ background: 'rgba(16,185,129,0.15)', color: '#6ee7b7', padding: '0.3rem 0.8rem', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 600, border: '1px solid rgba(16,185,129,0.2)' }}>✓ Free</span>}
            </div>
          </div>
        </div>
        ...
        <div style={{ background: 'rgba(31,41,55,0.9)', borderRadius: '20px', padding: '2rem', border: '1px solid rgba(59,130,246,0.15)', marginBottom: '1.5rem' }}>
          <h3 style={{ margin: '0 0 1.25rem', color: '#93c5fd', fontSize: '1rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1px' }}>
            Lesson Content
          </h3>
          <div style={{ lineHeight: 2, fontSize: '1rem', color: 'rgba(255,255,255,0.85)' }}
            dangerouslySetInnerHTML={{ __html: safeContent(selected.content) }} />
        </div>

        <button onClick={() => nav("/quiz")}
          style={{ width: '100%', padding: '1.1rem', background: 'linear-gradient(135deg,#3b82f6,#8b5cf6)', color: 'white', border: 'none', borderRadius: '16px', fontWeight: 700, fontSize: '1.1rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', boxShadow: '0 8px 24px rgba(59,130,246,0.35)', transition: 'all 0.3s' }}
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#0f172a,#1e293b)', color: 'white', padding: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          {!isGuest && (
            <button onClick={() => nav("/learning-dashboard")} style={{ padding: '0.75rem 1.5rem', background: 'linear-gradient(45deg,#f59e0b,#d97706)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 600 }}>🔖 My Bookmarks</button>
          )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: '1.5rem' }}>
        {lessons.length === 0 ? (
          ...
          <div key={lesson._id} onClick={() => open(lesson)}
            ...
            {!isGuest && (
              <button onClick={(e) => toggleBookmark(e, lesson._id)}
                style={{ position: 'absolute', top: '1rem', right: isAdmin ? '5.5rem' : '1rem', background: 'none', border: 'none', cursor: 'pointer', fontSize: '1.2rem', opacity: bookmarks.includes(lesson._id) ? 1 : 0.4 }}>
                🔖
              </button>
            )}
