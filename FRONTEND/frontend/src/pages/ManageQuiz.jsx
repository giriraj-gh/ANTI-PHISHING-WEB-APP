...
export default function ManageQuiz() {
  ...
  const [form, setForm] = useState({ title: "", description: "", isDemo: false, questions: Array(3).fill(null).map(() => ({ question: "", options: ["", "", "", ""], correctAnswer: "" })) });
  ...
  const activateQuiz = async (id) => {
    try { await api.put(`/quiz/activate/${id}`); loadQuizzes(); }
    catch (e) { alert('Error launching quiz'); }
  };

  const activateAll = async () => {
    const waiting = quizzes.filter(q => q.status === 'waiting');
    for (const q of waiting) { await api.put(`/quiz/activate/${q._id}`); }
    loadQuizzes();
    alert(`✅ ${waiting.length} quizzes activated!`);
  };

  const saveQuiz = async () => {
    try {
      if (editId) { await api.put(`/quiz/${editId}`, form); }
      else { await api.post('/quiz/create', { ...form, status: 'waiting' }); }
      setShowForm(false); resetForm(); loadQuizzes();
    } catch (e) { alert('Error saving quiz'); }
  };

  const resetForm = () => {
    setForm({ title: "", description: "", isDemo: false, questions: Array(3).fill(null).map(() => ({ question: "", options: ["", "", "", ""], correctAnswer: "" })) });
    setEditId(null);
  };
  ...
  const editQuiz = (quiz) => { setForm(quiz); setEditId(quiz._id); setShowForm(true); };
      ...
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ background: 'rgba(31,41,55,0.9)', padding: '2rem', borderRadius: '20px', border: '1px solid rgba(139,92,246,0.2)', marginBottom: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <span style={{ background: viewQuiz.status === 'active' ? '#10b981' : '#f59e0b', color: 'white', padding: '0.2rem 0.6rem', borderRadius: '8px', fontSize: '0.8rem', fontWeight: 600 }}>{viewQuiz.status === 'active' ? '✅ Active' : '⏳ Waiting'}</span>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
              {q.options?.map((opt, j) => (
                <div key={j} style={{ padding: '0.75rem 1rem', borderRadius: '8px', border: `2px solid ${opt === q.correctAnswer ? '#10b981' : 'rgba(255,255,255,0.1)'}`, background: opt === q.correctAnswer ? 'rgba(16,185,129,0.15)' : 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <span>{opt}</span>
                  {opt === q.correctAnswer && <span style={{ marginLeft: 'auto', color: '#10b981', fontWeight: 700 }}>✓</span>}
                </div>
              ))}
            </div>
          </div>
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#1e1b4b,#312e81)', color: 'white', padding: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          ...
          <button onClick={() => setShowForm(true)} style={{ padding: '0.75rem 1.5rem', background: 'linear-gradient(45deg,#10b981,#059669)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 600 }}>+ Add Quiz</button>
        </div>
      </div>
      ...
      {showForm && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setShowForm(false)}>
          <div style={{ background: '#1f2937', padding: '2rem', borderRadius: '16px', maxWidth: 700, width: '100%', maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            <h2 style={{ margin: '0 0 1.5rem' }}>{editId ? 'Edit Quiz' : 'Add New Quiz'}</h2>
            ...
            <h3 style={{ margin: '0 0 1rem' }}>Questions</h3>
            {form.questions.map((q, i) => (
              <div key={i} style={{ background: 'rgba(0,0,0,0.2)', padding: '1rem', borderRadius: '8px', marginBottom: '1rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                  <strong>Q{i + 1}</strong>
                  {form.questions.length > 1 && <button onClick={() => setForm({ ...form, questions: form.questions.filter((_, j) => j !== i) })} style={{ background: '#dc2626', color: 'white', border: 'none', borderRadius: '50%', width: 28, height: 28, cursor: 'pointer' }}>✕</button>}
                </div>
                ...
                <select value={q.correctAnswer} onChange={e => updateQuestion(i, 'correctAnswer', e.target.value)}
                  style={{ width: '100%', padding: '0.6rem', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '6px', background: 'rgba(0,0,0,0.3)', color: 'white' }}>
            ...
            <button onClick={() => setForm({ ...form, questions: [...form.questions, { question: "", options: ["", "", "", ""], correctAnswer: "" }] })}
              style={{ width: '100%', padding: '0.75rem', background: 'rgba(59,130,246,0.2)', color: 'white', border: '1px solid #3b82f6', borderRadius: '8px', cursor: 'pointer', marginBottom: '1rem' }}>+ Add Question</button>
            <div style={{ display: 'flex', gap: '1rem' }}>
              <button onClick={saveQuiz} style={{ flex: 1, padding: '0.75rem', background: 'linear-gradient(45deg,#10b981,#059669)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 600 }}>Save Quiz</button>
              <button onClick={() => { setShowForm(false); resetForm(); }} style={{ flex: 1, padding: '0.75rem', background: 'rgba(255,255,255,0.1)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 600 }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {quizzes.map(quiz => (
          <div key={quiz._id} style={{ background: 'rgba(31,41,55,0.8)', padding: '1.5rem', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
            <div>
              <h3 style={{ margin: '0 0 0.25rem' }}>{quiz.title}</h3>
              <p style={{ margin: '0 0 0.75rem', opacity: 0.7, fontSize: '0.9rem' }}>{quiz.description}</p>
              <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                <span style={{ background: quiz.status === 'active' ? '#10b981' : '#f59e0b', color: 'white', padding: '0.2rem 0.6rem', borderRadius: '12px', fontSize: '0.8rem', fontWeight: 600 }}>{quiz.status === 'active' ? '✅ Active' : '⏳ Waiting'}</span>
                <span style={{ background: quiz.isDemo ? '#f59e0b' : '#8b5cf6', color: 'white', padding: '0.2rem 0.6rem', borderRadius: '12px', fontSize: '0.8rem', fontWeight: 600 }}>{quiz.isDemo ? 'DEMO' : 'PREMIUM'}</span>
                <span style={{ opacity: 0.7, fontSize: '0.85rem' }}>📊 {quiz.questions?.length || 0} Questions</span>
              </div>
            </div>
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              ...
              {quiz.status === 'waiting' && <button onClick={() => activateQuiz(quiz._id)} style={{ padding: '0.5rem 1rem', background: '#10b981', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 600 }}>🚀 Launch</button>}
